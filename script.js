...
function initEnemies() {
    ...
    enemies.forEach(en => {
        ...
        en.walkFrames = 8; en.attackFrames = 4; en.deadFrames = 3; en.hurtFrames = 6;
...
// --- SELEÇÃO DE PERSONAGEM (A MÁGICA PARA MOBILE 2025) ---
window.escolherPersonagem = function(genero) {
    ...
    // 1. Remove o menu do DOM para liberar cliques
    const menu = document.getElementById('selection-menu');
    if(menu) menu.remove();

    // 2. Força a exibição dos controles com prioridade máxima
    ...
    // 3. O SEGREDO: Força o navegador a recalcular a tela
...
window.pular = function() {
    if (gameState === 'playing' && player.onGround && player.state !== 'dead') {
...
function update() {
    ...
    enemies.forEach(en => {
        ...
        if(dist < 50 && Math.abs(player.y - en.y) < 50) takeDamage(en.damage);
    });
...
function draw() {
    ...
    enemies.concat(player).forEach(obj => {
        ...
        if (img.complete) {
            ...
            obj.frameTimer++;
            if(obj.frameTimer > obj.frameInterval) {
                obj.currentFrame = (obj.currentFrame + 1) % frames;
                obj.frameTimer = 0;
                if(obj.state === 'attacking' && obj.currentFrame === frames - 1) obj.state = 'normal';
                if(obj.state === 'hurt' && obj.currentFrame === frames - 1) obj.state = 'normal';
            }
            ctx.save();
            if(isP && obj.invincible && Math.floor(Date.now()/100)%2===0) ctx.globalAlpha = 0.5;
            if(obj.facing === 'left') {
                ctx.translate(obj.x + obj.width, obj.y); ctx.scale(-1, 1);
                ctx.drawImage(img, obj.currentFrame * fw, 0, fw, img.height, 0, 0, obj.width, obj.height);
            } else {
                ctx.drawImage(img, obj.currentFrame * fw, 0, fw, img.height, obj.x, obj.y, obj.width, obj.height);
            }
            ctx.restore();
        }
    });
