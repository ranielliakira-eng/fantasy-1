...
const platforms = [
    ...
    { x: 6400, y: 280, w: 500, h: 20 } // Altar da Enchantress
];
...
let enemies = [];
function initEnemies() {
    // Definimos os inimigos um pouco acima do chão (y: 200) para garantir que não caiam do mapa
    enemies = [
        { type: 'Green_Slime', x: 600, y: 200, hp: 1, speed: 1.2, range: 200, damage: 1 },
        { type: 'Red_Slime', x: 1500, y: 200, hp: 1, speed: 2.5, range: 450, damage: 1 },
        { type: 'Blue_Slime', x: 2500, y: 200, hp: 1, speed: 1.8, range: 250, damage: 1 },
        { 
            type: 'Enchantress', x: 6600, y: 150, hp: 3, speed: 2, 
            attackType: 'melee', range: 400, damage: 1,
            dialogs: [
...
window.escolherPersonagem = function(genero) {
    ...
    // Mostra interface e esconde menu
...
function draw() {
    ...
    enemies.concat(player).forEach(obj => {
        ...
        if (img && img.complete && img.width > 0) {
            ...
            // Desenha um quadrado de backup caso a imagem falhe (Ajuda a debugar)
            ctx.fillStyle = "red";
