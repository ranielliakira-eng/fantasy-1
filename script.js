...
window.pular = function() {
    if(gameState === 'playing' && player.state === 'normal' && player.onGround) player.velY = player.jumpForce;
};
        ...
        if(dist < 90 && Math.abs(player.y - en.y) < 60) {
            if((player.facing === 'right' && en.x > player.x) || (player.facing === 'left' && en.x < player.x)) {
                ...
                if(en.hp <= 0) {
                    ...
                    en.state = 'hurt'; en.currentFrame = 0; en.velX = (player.x < en.x) ? 5 : -5;
                }
            }
        }
    });
...
function update() {
    ...
    if(player.x < 0) player.x = 0; if(player.x > mapWidth - player.width) player.x = mapWidth - player.width;
    ...
    if(player.frameTimer >= player.frameInterval) {
        ...
        if(player.state === 'attacking') {
            ...
            player.currentFrame++; if(player.currentFrame >= player.hurtFrames) { player.state = 'normal'; player.currentFrame = 0; }
        } else if(Math.abs(player.velX) > 0.1 && player.onGround) {
